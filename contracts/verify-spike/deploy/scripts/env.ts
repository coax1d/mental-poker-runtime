...
export const SPIKE_DIR = resolve(DEPLOY_DIR, "..");
export const CONTRACT_PVM = resolve(SPIKE_DIR, "contract.polkavm");
...
export function readContractCode(): Uint8Array {
  if (!existsSync(CONTRACT_PVM)) {
    throw new Error(
      `contract.polkavm not found at ${CONTRACT_PVM}. Run \`make\` in ${SPIKE_DIR}.`,
    );
  }
